...
/**********************************************************************/
  ...
  if(state == GTK_STATE_SELECTED){
    ...
    if(!new_item && active_item.type != GTK_PLOT_CANVAS_TEXT &&
       ...
       active_item.type != GTK_PLOT_CANVAS_POLYGON &&
...
gboolean
gtk_plot_canvas_get_child_position(GtkPlotCanvas *canvas,
                             GtkPlotCanvasChild *child,
			     double *x1, double *y1,
			     double *x2, double *y2)
{
...
gpointer
gtk_plot_canvas_get_child_data(GtkPlotCanvasChild *child)
{
   return child->data;
}

GtkPlotCanvasType
gtk_plot_canvas_get_child_type(GtkPlotCanvasChild *child)
{
   return child->type;
}
...
GtkPlotCanvasChild *
gtk_plot_canvas_put_polygon(GtkPlotCanvas *canvas,
                            GtkPlotPoint *cpoints,
                            guint npoints,
		  	    GtkPlotLineStyle style,
			    gfloat width,
			    const GdkColor *fg,
			    ...
			    gboolean fill)
{
  ...
  GtkPlotCanvasChild *child;
  GtkPlotCanvasPolygon *poly;
  double x1, x2, y1, y2;
  guint i;

  child = gtk_plot_canvas_child_new(GTK_PLOT_CANVAS_POLYGON);
  poly = (GtkPlotCanvasPolygon*)child->data;

  gdk_color_black(gdk_colormap_get_system(), &poly->line.color); 
  gdk_color_white(gdk_colormap_get_system(), &poly->bg); 
  gtk_plot_canvas_polygon_set_attributes(child, style, width, fg, bg, fill);

  /* Establish bounding box in canvas space */
  x1 = x2 = cpoints[0].x;
  y1 = y2 = cpoints[0].y;
  for (i=1; i<npoints; i++) {
	if (cpoints[i].x < x1) x1 = cpoints[i].x;
	if (cpoints[i].x > x2) x2 = cpoints[i].x;
	if (cpoints[i].y < y1) y1 = cpoints[i].y;
	if (cpoints[i].y > y2) y2 = cpoints[i].y;
  }

  /* Establish pixel space coordinates and dimensions.  Subtract x1,y1
   * so that pixel values are relative to child location on canvas, not
   * canvas origin.  This simplifies any later moving/scaling */
  poly->points = g_new(GtkPlotPoint, npoints);
  for (i=0; i<npoints; i++) {
	poly->points[i].x = (cpoints[i].x - x1) * canvas->pixmap_width;
	poly->points[i].y = (cpoints[i].y - y1) * canvas->pixmap_height;
  }
  poly->width  = x2 - x1;
  poly->height = y2 - y1;
...
void
gtk_plot_canvas_polygon_set_attributes(GtkPlotCanvasChild *child,
		  	               GtkPlotLineStyle style,
			               gfloat width,
			               const GdkColor *fg,
			               const GdkColor *bg,
			               gboolean fill)
{
  ...
  g_return_if_fail(child->type == GTK_PLOT_CANVAS_POLYGON);
  ...
  if(bg)
    poly->bg = *bg;
  poly->line.line_width = width;
  poly->line.line_style = style;
  poly->filled = fill;
}
...
gtk_plot_canvas_child_new(GtkPlotCanvasType type)
{
  ...
  switch(type){
	...
	child->data = g_new0(GtkPlotCanvasEllipse, 1);
        break;    
    case GTK_PLOT_CANVAS_POLYGON:
	child->data = g_new0(GtkPlotCanvasPolygon, 1);
        break;    
    case GTK_PLOT_CANVAS_TEXT:
  ...
  while(list)
   {
     ...
     if((GtkPlotCanvasChild *)data == child){
	  ...
	  switch(child->type) {
	     case GTK_PLOT_CANVAS_PIXMAP:
		gdk_pixmap_unref( (GdkPixmap *)child->data);
		break;
	     case GTK_PLOT_CANVAS_TEXT:
		g_free( ((GtkPlotText *)child->data)->font);
		g_free( ((GtkPlotText *)child->data)->text);
		g_free(child->data);
		break;
	     case GTK_PLOT_CANVAS_POLYGON:
		g_free( ((GtkPlotCanvasPolygon*)child->data)->points);
		g_free(child->data);
		break;
	     default:
	        g_free(child->data);
                           ...
                           GtkPlotCanvasChild *child)
{
  ...
  GtkPlotCanvasPolygon *poly = NULL;
  ...
  switch(child->type){
    ...
    case GTK_PLOT_CANVAS_POLYGON:
        poly = (GtkPlotCanvasPolygon*)child->data;
        scale_x = fabs(child->rx1 - child->rx2) / poly->width;
        scale_y = fabs(child->ry1 - child->ry2) / poly->height;
	points = g_new(GtkPlotPoint,poly->npoints);

	for (i=0; i< poly->npoints; i++) {
	    points[i].x = rx1 + (poly->points[i].x * scale_x);
	    points[i].y = ry1 + (poly->points[i].y * scale_y);
	}

        if(poly->filled){
           gtk_plot_pc_set_color(canvas->pc, &poly->bg);
           gtk_plot_pc_draw_polygon(canvas->pc,TRUE,points,poly->npoints);
        }
        gtk_plot_canvas_set_line_attributes(canvas, poly->line);
        if(poly->line.line_style != GTK_PLOT_LINE_NONE)
           gtk_plot_pc_draw_polygon(canvas->pc,FALSE,points,poly->npoints);
